...
# 시간측정 및 손포즈 인식 ON/OFF
span_sec = 5.0 # 5.0 sec
flag_hands_poses = False # 초기설정
...
try:
    while (True):
        ...
        if hands:
            for hd in hands:  # 최대 2개의 손을 인식
                ...
                if len(lmList) != 0: # lmList의1번째값이 1값
                    ...
                    # handpose_marker를 표시
                    cv2.putText(img, f"{handpose_marker}", (x, y+height), cv2.FONT_HERSHEY_PLAIN, 2, color, 2)
                    
                    if flag_hands_poses:
                        cv2.putText(img, f"ON", (20,20), cv2.FONT_HERSHEY_PLAIN, 2, color, 2)
                    
        
            # Tello Drone 제어하기
            if flag_hands_poses:  # ON
        ...
        if  key == ord('q') or key == ord('x'):
            ...
            flag_hands_poses = True
        elif key == ord('2'): # off
            flag_hands_poses = False
